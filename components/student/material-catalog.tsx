      ...
      <div className="flex-1 bg-muted/50">
        <div className="container mx-auto px-4 py-8">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Explora y solicita materiales de laboratorio</p>
          </div>
          ...
          {selectedMaterials.size > 0 && (
            <Card className="mb-6 border-primary">
              <CardHeader>
                <CardTitle>Materiales Seleccionados ({selectedMaterials.size})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {Array.from(selectedMaterials.entries()).map(([materialId, quantity]) => {
                    const material = materials.find((m) => m.id === materialId)
                    ...
                    return (
                      <div key={materialId} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Button size="sm" variant="outline" onClick={() => updateQuantity(materialId, quantity - 1)}>
                            -
                          </Button>
                          ...
                          <Button size="sm" variant="ghost" onClick={() => toggleMaterial(materialId)}>
                            Quitar
                          </Button>
                        </div>
                      </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredMaterials.map((material) => (
              <Card key={material.id} className={selectedMaterials.has(material.id) ? "border-primary" : ""}>
                <CardHeader>
                ...
                <CardFooter>
                  <Button
                    variant={selectedMaterials.has(material.id) ? "secondary" : "default"}
                    className="w-full"
